...
void Ctags::addcelldata(UINT8* row, UINT8* col, UINT8* t, UINT8* s, UINT8* v, F* f, UINT8* si) {
    ...
    if (!ro) {
        std::cout << "row なし 追加 " <<rn << std::endl;
        int cnpl = 0;//桁数　使わない
        ...
        while (incolnum <= cn) {
            if (incolnum == cn) {
                ...
                UINT8 news[] = "1023";//ノーマルなスタイル
                UINT8* ws = (UINT8*)malloc(5);
                strcpy_s((char*)ws,5, (char*)news);

                ro->cells = addCtable(ro->cells, n1, ws, n2, incolnum, n3, n4);
            }
            NA.ColumnIncliment(&incolnum);
            //colnum_start++;
        }
