...
bool searchItemNum::searchitemNumber(UINT8* uniq,char* one,char* two,char*three,char* four,char* style) {
    ...
    if (!incellflag)
        ...
        Cels->addcelldata(nr, incolumn, (UINT8*)SaT[0], (UINT8*)style, uniq, nullptr, nullptr);//最初の一回に変更 メインの日付追加
        //サブ文字列追加
        if (one) {
            rowslide--;
            UINT8* srow = changenum.InttoChar(rowslide, &result);
            Cels->addcelldata(srow, incolumn, (UINT8*)SaT[0], (UINT8*)style, (UINT8*)one, nullptr, nullptr);
        }
        if (two) {
            rowslide--;
            UINT8* srow = changenum.InttoChar(rowslide, &result);
            Cels->addcelldata(srow, incolumn, (UINT8*)SaT[0], (UINT8*)style, (UINT8*)two, nullptr, nullptr);
        }
        if (three) {
            rowslide--;
            UINT8* srow = changenum.InttoChar(rowslide, &result);
            Cels->addcelldata(srow, incolumn, (UINT8*)SaT[0], (UINT8*)style, (UINT8*)three, nullptr, nullptr);
        }
        if (four) {
            rowslide--;
            UINT8* srow = changenum.InttoChar(rowslide, &result);
            Cels->addcelldata(srow, incolumn, (UINT8*)SaT[0], (UINT8*)style, (UINT8*)four, nullptr, nullptr);
        }
    }
                            ...
                            for (int i = 0; i < 9; i++) {//サイズループ
                                ...
                                if (result == 0) {//サイズ一致
                                    ...
                                    if (sizeMatch) {
                                        ...
                                        Cels->addcelldata(nr, incolumn, nullptr, (UINT8*)s, sizeMatch, nullptr, nullptr);
